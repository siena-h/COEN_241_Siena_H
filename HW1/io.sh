...
#Run format: sh io.sh <delay> <file size>
#Date: 01/29/23
...
while [ $i -lt $iter ]
do
	sh mode.sh seqwr $2 > seqwr$2$i.txt
	i=$(($i+$inc))
...
while [ $i -lt $iter ]
do
	sh mode.sh seqrd $2 > seqrd$2$i.txt
	echo "Iteration ending. Sync now!\n"
	sleep $1
	i=$(($i+$inc))
done
...
while [ $i -lt $iter ]
do
	sh mode.sh rndrd $2 > rndrd$2$i.txt
	echo "Iteration ending. Sync now!\n"
	sleep $1
	i=$(($i+$inc))
done
...
while [ $i -lt $iter ]
do
	sh mode.sh rndwr $2 > rndwr$2$i.txt
	echo "Iteration ending. Sync now!\n"
	i=$(($i+$inc))
	sleep $1
done
